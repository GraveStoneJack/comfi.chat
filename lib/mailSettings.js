...
function sanitize(settings) {
	...
	return {
		...
		authMethod: settings.authMethod || 'LOGIN',
...
async function saveSettings(payload, adminId) {
	...
	const update = {
		...
		authMethod: ['LOGIN', 'PLAIN'].includes(payload.authMethod) ? payload.authMethod : 'LOGIN',
	...
	if (payload.password) {
		update.passwordEncrypted = encryptSecret(String(payload.password).trim());
